 ...
 * {@link #forNames(Iterable, ClassLoader...)}
 * <p>
...
public abstract class ReflectionUtils {
    ...
    public static <T extends AnnotatedElement> Set<T> getAll(final Set<T> elements, Predicate<? super T>... predicates) {
        Iterator parametros = Arrays.stream(predicates).iterator();
        return isEmpty(predicates) ? elements : Sets.newHashSet(Iterables.filter(elements,
                Predicates.and(iteratorToIterable(parametros))));
    ...
    static <T> Set<T> filter(final T[] elements, Predicate<? super T>... predicates) {
        ...
        return isEmpty(predicates) ? Sets.newHashSet(elements)
                : Sets.newHashSet(Iterables.filter(Arrays.asList(elements), Predicates.and(iteratorToIterable(parametros))));
    }

    static <T> Set<T> filter(final Iterable<T> elements, Predicate<? super T>... predicates) {
        Iterator parametros = Arrays.stream(predicates).iterator();
        return isEmpty(predicates) ? Sets.newHashSet(elements)
                : Sets.newHashSet(Iterables.filter(elements, Predicates.and(iteratorToIterable(parametros))));
    }
